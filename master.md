# 🛡️ TAKARA MASTER PROMPT (v6) — CRITICAL SECURITY & ARCHITECTURE FIX
...
Этот документ содержит **финальные и обязательные** исправления для проекта TAKARA. Все предыдущие версии ТЗ (v4, v5) отменяются в части противоречий с данным документом.

### Критические проблемы для исправления:

1. **❌ Неправильные APY** — Vault 2 и Vault 3 имеют неверные стартовые проценты
2. **❌ Отсутствует мульти-чейн депозит** — нет логики последовательного пополнения USDT (Tron) + Takara/Laika (Solana)
3. **❌ Wexel не реализован как NFT** — вексель должен быть Solana NFT, подтверждающим право собственности
4. **❌ Нет верификации владения** — отсутствует криптографическая проверка права на claim rewards
5. **❌ Уязвимости безопасности** — возможна подделка векселя, нет защиты от replay-атак

---

## 🔢 ЧАСТЬ 1: ИСПРАВЛЕНИЕ ЭКОНОМИКИ (APY CORRECTION)

### ⚠️ ВАЖНО: Эти значения являются ФИНАЛЬНЫМИ и должны быть применены везде: БД, бэкенд, фронтенд, смарт-контракты.
...
## 🔐 ЧАСТЬ 2: АРХИТЕКТУРА WEXEL (NFT-ВЕКСЕЛЬ)

### 2.1 Концепция

**Wexel (Вексель)** — это Solana NFT (Metaplex Core Standard), который является **ЕДИНСТВЕННЫМ** подтверждением права собственности на депозит.

```
┌─────────────────────────────────────────────────────────────┐
│                      WEXEL NFT                              │
│  ┌───────────────────────────────────────────────────────┐  │
│  │  Token Standard: Metaplex Core (или Token-2022)       │  │
│  │  Mint Authority: TAKARA Program (PDA)                 │  │
│  │  Update Authority: TAKARA Program (PDA)               │  │
│  │  Transferable: YES (вторичный рынок)                  │  │
│  │  Burnable: YES (при погашении депозита)               │  │
│  └───────────────────────────────────────────────────────┘  │
│                                                             │
│  ON-CHAIN METADATA (immutable):                             │
│  ├── deposit_id: u64                                        │
│  ├── vault_type: u8 (1, 2, or 3)                           │
│  ├── deposit_amount_usdt: u64 (lamports precision)         │
│  ├── deposit_currency: "USDT_TRC20"                        │
│  ├── tron_tx_hash: String (32 bytes hex)                   │
│  ├── base_apy: u16 (basis points, e.g., 700 = 7%)          │
│  ├── boost_apy: u16 (basis points)                         │
│  ├── total_apy: u16 (basis points)                         │
│  ├── lock_period_months: u8                                │
│  ├── lock_start_timestamp: i64                             │
│  ├── lock_end_timestamp: i64                               │
│  ├── payout_frequency: u8 (1=monthly, 3=quarterly, 12=yr)  │
│  ├── boost_token_mint: Pubkey (Laika or Takara)            │
│  ├── boost_token_amount: u64                               │
│  ├── boost_token_locked: bool                              │
│  └── created_at: i64                                       │
│                                                             │
│  MUTABLE STATE (PDA Account):                               │
│  ├── total_claimed_usdt: u64                               │
│  ├── total_claimed_takara: u64                             │
│  ├── last_claim_timestamp: i64                             │
│  └── is_redeemed: bool                                     │
└─────────────────────────────────────────────────────────────┘
```

### 2.2 Правила владения
...
├────────────────────────────────────────────────────────────────┤
│                                                                 │
│  RULE 1: NFT Owner = Deposit Owner                             │
│  ─────────────────────────────────────────────────────────────│
│  Кошелёк, на котором находится Wexel NFT, является             │
│  ЕДИНСТВЕННЫМ владельцем депозита и имеет право:               │
│  • Клеймить USDT rewards                                       │
│  • Клеймить Takara mining rewards                              │
│  • Погасить депозит (burn NFT → получить principal)            │
│                                                                 │
│  RULE 2: Transfer = Transfer of Rights                         │
│  ─────────────────────────────────────────────────────────────│
│  При продаже/передаче NFT на вторичном рынке:                  │
│  • Старый владелец ТЕРЯЕТ все права                            │
│  • Новый владелец ПОЛУЧАЕТ все права автоматически             │
│  • Backend должен отслеживать transfers и обновлять owner      │
│                                                                 │
│  RULE 3: No NFT = No Rights                                    │
│  ─────────────────────────────────────────────────────────────│
│  Без NFT в кошельке пользователь НЕ МОЖЕТ:                     │
│  • Видеть депозит в Dashboard                                  │
│  • Клеймить любые награды                                      │
│  • Погасить депозит                                            │
│                                                                 │
└────────────────────────────────────────────────────────────────┘
```

### 2.3 Безопасность Wexel
...
├────────────────────────────────────────────────────────────────┤
│                                                                 │
│  🛡️ ANTI-FORGERY (Защита от подделки)                          │
│  ──────────────────────────────────────────────────────────────│
│  1. Только TAKARA Program может минтить Wexel NFT              │
│  2. Mint Authority = Program PDA (не EOA кошелёк!)             │
│  3. Collection NFT подписан Program Authority                  │
│  4. Backend верифицирует: Collection Address + Creator         │
│                                                                 │
│  🔒 CLAIM VERIFICATION (Верификация клейма)                    │
│  ──────────────────────────────────────────────────────────────│
│  ON-CHAIN (Smart Contract):                                    │
│  • Проверить: signer == token_account.owner                    │
│  • Проверить: nft.collection == TAKARA_COLLECTION              │
│  • Проверить: nft.creators[0].verified == true                 │
│  • Проверить: wexel_state.is_redeemed == false                 │
│                                                                 │
│  OFF-CHAIN (Backend):                                          │
│  • Верифицировать подпись сообщения владельцем NFT             │
│  • Проверить ownership через RPC getTokenAccountsByOwner       │
│  • Cross-check с БД (deposit_id, amounts, timestamps)          │
│                                                                 │
│  🚫 REPLAY ATTACK PROTECTION                                   │
│  ──────────────────────────────────────────────────────────────│
│  • Каждый claim включает nonce (инкрементируемый счётчик)      │
│  • Timestamp validation (не старше 5 минут)                    │
│  • Unique message per claim: `claim:${wexel_id}:${nonce}:${ts}`│
│                                                                 │
│  🔥 BURN ON REDEEM                                             │
│  ──────────────────────────────────────────────────────────────│
│  При погашении депозита:                                       │
│  • NFT сжигается (burn)                                        │
│  • Boost токены разлочиваются и возвращаются владельцу         │
│  • wexel_state.is_redeemed = true                              │
│  • Principal USDT отправляется на Tron кошелёк владельца       │
│                                                                 │
└────────────────────────────────────────────────────────────────┘
```
...
## 🔄 ЧАСТЬ 3: МУЛЬТИ-ЧЕЙН ДЕПОЗИТ (TRON + SOLANA)
...
│                                                                          │
│  Frontend Actions:                                                       │
│  1. Проверить наличие TronLink extension                                │
│  2. Проверить наличие Phantom/Solflare extension                        │
│  3. Запросить подключение ОБОИХ кошельков                               │
│  4. Получить и сохранить:                                               │
│     • tronAddress: string (base58)                                      │
│     • solanaAddress: PublicKey                                          │
│                                                                          │
│  Validation:                                                             │
│  • Оба кошелька ОБЯЗАТЕЛЬНЫ для продолжения                             │
│  • Показать ошибку если один из кошельков не подключен                  │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
                                   │
                                   ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ STEP 1: USDT DEPOSIT (TRON BLOCKCHAIN)                                   │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  User Action: Отправить USDT TRC20 на Treasury                          │
│                                                                          │
│  Frontend:                                                               │
│  1. Показать форму с суммой депозита                                    │
│  2. Валидация: amount >= vault.minEntry                                 │
│  3. Вызвать TronLink:                                                   │
│     ```javascript                                                        │
│     const tronWeb = window.tronWeb;                                     │
│     const usdtContract = await tronWeb.contract().at(USDT_TRC20_ADDR);  │
│     const tx = await usdtContract.transfer(                             │
│       TAKARA_TREASURY_TRON,                                             │
│       amount * 1e6  // USDT has 6 decimals                              │
│     ).send();                                                           │
│     ```                                                                  │
│  4. Ждать подтверждения (≥19 confirmations)                             │
│  5. Сохранить: tronTxHash                                               │
│                                                                          │
│  Backend Verification:                                                   │
│  1. Получить tronTxHash от фронтенда                                    │
│  2. Запросить Tron RPC: getTransactionInfo(txHash)                      │
│  3. Проверить:                                                          │
│     • tx.to == TAKARA_TREASURY_TRON                                     │
│     • tx.contractAddress == USDT_TRC20_ADDR                             │
│     • tx.amount >= minEntry                                             │
│     • tx.status == SUCCESS                                              │
│     • tx.confirmations >= 19                                            │
│  4. Создать pending deposit record в БД                                 │
│                                                                          │
│  Status: PENDING_BOOST (если буст выбран) или PENDING_MINT              │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
                                   │
                                   ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ STEP 2: BOOST TOKEN LOCK (SOLANA BLOCKCHAIN) — OPTIONAL                  │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  ⚠️ Этот шаг ПРОПУСКАЕТСЯ если пользователь не выбрал буст              │
│                                                                          │
│  User Action: Залочить Laika или Takara на смарт-контракт               │
│                                                                          │
│  Расчёт требуемой суммы:                                                │
│  ┌────────────────────────────────────────────────────────────────────┐ │
│  │ Vault 1 (Laika):                                                   │ │
│  │   requiredLaika = (depositUSDT * 0.40) / (laikaPrice * 0.85)      │ │
│  │   где laikaPrice = CoinGecko price, 0.85 = 15% discount           │ │
│  │                                                                    │ │
│  │ Vault 2 & 3 (Takara):                                             │ │
│  │   requiredTakara = depositUSDT / 0.10                             │ │
│  │   где 0.10 = фиксированный курс Takara                            │ │
│  │   Пример: $1500 депозит → 15,000 Takara tokens                    │ │
│  └────────────────────────────────────────────────────────────────────┘ │
│                                                                          │
│  Frontend:                                                               │
│  1. Показать требуемую сумму boost токенов                              │
│  2. Проверить баланс пользователя                                       │
│  3. Вызвать Solana Program:                                             │
│     ```typescript                                                        │
│     const tx = await program.methods                                    │
│       .lockBoost(new BN(boostAmount))                                   │
│       .accounts({                                                       │
│         user: wallet.publicKey,                                         │
│         userTokenAccount: userBoostATA,                                 │
│         vaultTokenAccount: vaultBoostATA,                               │
│         boostMint: LAIKA_MINT || TAKARA_MINT,                          │
│         depositState: depositPDA,                                       │
│         tokenProgram: TOKEN_PROGRAM_ID,                                 │
│       })                                                                │
│       .rpc();                                                           │
│     ```                                                                  │
│  4. Ждать finalization                                                  │
│  5. Сохранить: solanaTxSignature                                        │
│                                                                          │
│  Backend Verification:                                                   │
│  1. Получить solanaTxSignature                                          │
│  2. Запросить Solana RPC: getTransaction(signature)                     │
│  3. Проверить:                                                          │
│     • Инструкция == lockBoost                                           │
│     • amount >= requiredBoostAmount                                     │
│     • destination == vault PDA                                          │
│  4. Обновить deposit record: boost_locked = true                        │
│                                                                          │
│  Status: PENDING_MINT                                                    │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
                                   ...
                                   ▼
┌─────────────────────────────────────────────────────────────────────────┐
│ STEP 3: WEXEL NFT MINTING (SOLANA BLOCKCHAIN)                            │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  Backend Preparation:                                                    │
│  1. Собрать все данные депозита:                                        │
│     ```typescript                                                        │
│     const depositData = {                                               │
│       depositId: deposit.id,                                            │
│       vaultType: deposit.vaultId,                                       │
│       amountUsdt: deposit.amountUsdt,                                   │
│       tronTxHash: deposit.tronTxHash,                                   │
│       baseApy: vault.baseApy,                                           │
│       boostApy: deposit.boostLocked ? vault.boostApy : 0,               │
│       lockPeriodMonths: vault.durationMonths,                           │
│       payoutFrequency: deposit.payoutFrequency,                         │
│       boostTokenMint: deposit.boostMint,                                │
│       boostTokenAmount: deposit.boostAmount,                            │
│       userSolanaWallet: deposit.solanaAddress,                          │
│     };                                                                  │
│     ```                                                                  │
│  2. Создать подпись для минтинга:                                       │
│     ```typescript                                                        │
│     const message = Buffer.from(JSON.stringify(depositData));           │
│     const signature = nacl.sign.detached(message, BACKEND_KEYPAIR);     │
│     ```                                                                  │
│  3. Вернуть signature + depositData на фронтенд                         │
│                                                                          │
│  Frontend:                                                               │
│  1. Получить signature от бэкенда                                       │
│  2. Вызвать Solana Program:                                             │
│     ```typescript                                                        │
│     const tx = await program.methods                                    │
│       .mintWexel(depositData, backendSignature)                         │
│       .accounts({                                                       │
│         user: wallet.publicKey,                                         │
│         wexelMint: wexelMintKeypair.publicKey,                         │
│         wexelMetadata: metadataPDA,                                     │
│         wexelState: wexelStatePDA,                                      │
│         collection: TAKARA_COLLECTION,                                  │
│         backendSigner: BACKEND_PUBKEY,                                  │
│         metadataProgram: METADATA_PROGRAM_ID,                           │
│         tokenProgram: TOKEN_PROGRAM_ID,                                 │
│         systemProgram: SystemProgram.programId,                         │
│       })                                                                │
│       .signers([wexelMintKeypair])                                      │
│       .rpc();                                                           │
│     ```                                                                  │
│  3. Показать успех + ссылку на NFT                                      │
│                                                                          │
│  Smart Contract Verification (внутри mintWexel):                        │
│  1. Verify backend signature                                            │
│  2. Verify deposit data integrity                                       │
│  3. Verify collection authority                                         │
│  4. Create Wexel NFT with metadata                                      │
│  5. Create WexelState PDA                                               │
│  6. Emit event: WexelMinted                                             │
│                                                                          │
│  Backend Post-Mint:                                                      │
│  1. Получить событие WexelMinted                                        │
│  2. Обновить БД: deposit.status = ACTIVE, deposit.wexelMint = mint      │
│                                                                          │
│  Status: ACTIVE                                                          │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
...
└─────────────────────────────────────────────────────────────────────────┘
  ...
  ┌───────────┐     USDT TX      ┌──────────────────┐
  │  INITIAL  │ ───────────────► │ PENDING_BOOST    │ (если буст)
  └───────────┘    confirmed     └──────────────────┘
        │                               │
        │ USDT TX                       │ Boost TX
        │ (без буста)                   │ confirmed
...
## 🗄️ ЧАСТЬ 4: DATABASE SCHEMA (Prisma)
...
│   ├── decorators/
│   ├── guards/
│   │   ├── solana-signature.guard.ts    # Проверка подписи Solana
│   │   └── tron-signature.guard.ts      # Проверка подписи Tron
│   ├── interceptors/
│   └── filters/
├── modules/
│   ├── vaults/                          # Renamed from pools!
│   │   ├── vaults.module.ts
...
│   │   ├── deposits.service.ts
│   │   ├── deposit-flow.service.ts      # Multi-chain flow orchestration
│   │   └── dto/
│   │       ├── create-deposit.dto.ts
│   │       ├── confirm-tron-tx.dto.ts
│   │       ├── confirm-boost-tx.dto.ts
│   │       └── mint-wexel.dto.ts
│   ├── wexel/
│   │   ├── wexel.module.ts
│   │   ├── wexel.service.ts             # NFT operations
│   │   ├── wexel-verification.service.ts # Ownership verification
│   │   └── wexel-monitor.service.ts     # Transfer monitoring
│   ├── claims/
│   │   ├── claims.module.ts
...
│   │   ├── tron/
│   │   │   ├── tron.service.ts          # Tron RPC interactions
│   │   │   └── tron-monitor.service.ts  # TX confirmation monitoring
│   │   └── solana/
│   │       ├── solana.service.ts        # Solana RPC interactions
│   │       ├── solana-program.service.ts # Anchor program calls
│   │       └── solana-monitor.service.ts # Events monitoring
│   ├── price-feed/
│   │   ├── price-feed.module.ts
│   │   └── price-feed.service.ts        # CoinGecko integration
│   └── calculator/
│       ├── calculator.module.ts
│       └── yield-calculator.service.ts  # APY/reward calculations
└── prisma/
    ├── prisma.module.ts
    └── prisma.service.ts
```

### 5.2 Ключевые сервисы
...
## 🖥️ ЧАСТЬ 6: FRONTEND (Next.js)

### 6.1 Компоненты Deposit Wizard
...
### 6.2 Vault Card с правильными APY
...
## ⛓️ ЧАСТЬ 7: SMART CONTRACTS (Solana/Anchor)
...
└── src/
    ...
    │   ├── lock_boost.rs   # Lock Laika/Takara for boost
    │   ├── mint_wexel.rs   # Mint Wexel NFT
    │   ├── claim_usdt.rs   # Claim USDT rewards (signature)
    │   ├── claim_takara.rs # Claim Takara mining rewards
    │   ├── unlock_boost.rs # Unlock boost tokens on redeem
    │   └── burn_wexel.rs   # Burn NFT on redemption
    ├── state/
    │   ├── mod.rs
    │   ├── program_config.rs
    │   ├── wexel_state.rs  # PDA for each Wexel NFT
    │   └── boost_vault.rs  # Token vault for locked boost
    ├── errors.rs
    └── events.rs
```
...
### 7.5 Claim Takara Instruction (с проверкой владения)
...
## ✅ ЧАСТЬ 8: CHECKLIST ДЛЯ АГЕНТА

### 8.1 Database Tasks

- [ ] Обновить schema.prisma согласно ЧАСТИ 4
- [ ] Создать миграцию: `npx prisma migrate dev --name vault_refactor_v6`
- [ ] Обновить seed data с ПРАВИЛЬНЫМИ APY
...
- [ ] Переименовать все "Pool" → "Vault" в коде
- [ ] Создать DepositFlowService с мульти-чейн логикой
- [ ] Создать WexelVerificationService
- [ ] Создать YieldCalculatorService с правильными формулами
- [ ] Добавить TronService для верификации USDT транзакций
- [ ] Добавить SolanaService для взаимодействия с программой
- [ ] Добавить endpoint: POST /deposits/initialize
- [ ] Добавить endpoint: POST /deposits/:id/confirm-tron
- [ ] Добавить endpoint: POST /deposits/:id/confirm-boost
- [ ] Добавить endpoint: POST /deposits/:id/mint-signature
- [ ] Добавить endpoint: POST /deposits/:id/confirm-mint
- [ ] Добавить endpoint: POST /claims/:depositId/takara
- [ ] Добавить endpoint: POST /claims/:depositId/usdt
- [ ] Добавить guards для проверки подписей

### 8.3 Frontend Tasks

- [ ] Заменить все "Pool" → "Vault" в UI
- [ ] Заменить все "TUSD" → "TAKARA" в UI
- [ ] Обновить VaultCard с правильными APY
- [ ] Создать DepositWizard компонент (5 шагов)
- [ ] Добавить подключение двух кошельков (Tron + Solana)
- [ ] Реализовать USDT deposit step (TronLink)
- [ ] Реализовать Boost lock step (Phantom/Solflare)
- [ ] Реализовать Wexel mint step
- [ ] Обновить Dashboard для отображения депозитов по NFT
- [ ] Добавить Claim UI с проверкой ownership

### 8.4 Smart Contract Tasks

- [ ] Создать Anchor проект в /programs/takara
- [ ] Реализовать ProgramConfig state
- [ ] Реализовать WexelState PDA
- [ ] Реализовать lock_boost instruction
- [ ] Реализовать mint_wexel instruction
- [ ] Реализовать claim_takara instruction
- [ ] Реализовать redeem_wexel instruction
- [ ] Добавить все security checks согласно ЧАСТИ 7
- [ ] Написать тесты для всех instructions
- [ ] Deploy на devnet для тестирования
- [ ] Провести security audit

### 8.5 Security Verification

- [ ] Backend signature verification работает
- [ ] NFT ownership check работает
- [ ] Nonce anti-replay работает
- [ ] Timestamp validation работает
- [ ] Collection verification работает
- [ ] Невозможно подделать Wexel NFT
- [ ] Невозможно claim без ownership
...
2. **WEXEL = OWNERSHIP:**
   - Кто владеет NFT = кто владеет депозитом
   - Проверка ownership ОБЯЗАТЕЛЬНА для каждого claim

3. **MULTI-CHAIN FLOW:**
   - Step 1: USDT на Tron (TronLink)
   - Step 2: Boost на Solana (Phantom)
   - Step 3: Mint NFT на Solana (Phantom)

4. **SECURITY FIRST:**
   - Все claims требуют backend signature
   - Все claims требуют ownership verification
   - Все claims требуют nonce validation
